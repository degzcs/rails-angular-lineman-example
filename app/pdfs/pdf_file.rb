...
class PdfFile < Prawn::Document
  ...
  def generate_certificate_b_c(values , date)
    ...
    move_cursor_to 112
    text_box "#{values[:buyer][:num_rucom] || values[:buyer][:rucom_record]}" , :at => [350,cursor] , :width => 300, :height => 15 , :overflow => :shrink_to_fit
  end
  ...
  def generate_purchase_report(values,date)
    ...
    text_box "#{values[:buyer][:rucom_record]}", :at => [390,cursor], :width => 150, :size => 10, :height =>  10, :overflow => :shrink_to_fit
    move_cursor_to 626
