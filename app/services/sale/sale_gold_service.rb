class Sale::SaleGoldService
  attr_reader :sale_order, :selected_purchase_ids, :seller, :buyer, :performer_user, :order_hash, :gold_batch_hash
  ...
  def setup_sale_order!
    order_hash.merge!(type: 'sale', performer: performer_user)
    @sale_order = seller.sales.build(order_hash)
    sale_order.build_gold_batch(gold_batch_hash.deep_symbolize_keys)
    response[:success] = sale_order.save! # This save both the new sale and gold_batch
    sale_order
  end
