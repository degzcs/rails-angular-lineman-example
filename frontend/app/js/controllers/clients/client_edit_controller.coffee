...
angular.module('app').controller 'ClientEditCtrl', ($scope, $state, $stateParams, $window, ClientService, RucomService, LocationService,$mdDialog) ->
  #*** Loading Variables **** #
  ...
  $scope.selectedPopulationCenterChange = (population_center) ->
    if population_center
      ClientService.modelToUpdate.external_user.population_center_id = population_center.id
    else
  ...
  $scope.save = ->
    ...
    ClientService.modelToUpdate.external_user.phone_number = $scope.currentExternalUser.phone_number
    ClientService.modelToUpdate.external_user.email = $scope.currentExternalUser.email
    ClientService.modelToUpdate.external_user.address = $scope.currentExternalUser.address
    
    ClientService.update_external_user($scope.currentExternalUser.id).success (data)->
      #$mdDialog.cancel()
      $state.go "show_client", {id: $scope.currentExternalUser.id}
