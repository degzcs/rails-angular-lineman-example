...
describe 'all test the transaction_movements view', :js do
  ...
  it 'New TransactionMovement' do
    ...
    expected_response = {
      ...
      accounting_entry: { value: 'D', select_name: TransactionMovement::ACCOUNTING_ENTRIES[:D] }
    }
    ...
    select(expected_response[:accounting_entry][:select_name], from: 'transaction_movement_accounting_entry')
    ...
    expect(last_tax_rule[:accounting_entry]).to eq expected_response[:accounting_entry][:value]
    expect(page).to have_content 'Transaction movement was successfully created.'
  end

  it 'Edit TransactionMovement' do
    expected_response = {
      accounting_entry: { value: 'C', select_name: TransactionMovement::ACCOUNTING_ENTRIES[:C] }
    }
    transaction_movement = TransactionMovement.create(puc_account_id: PucAccount.last.id , type: 'sale', block_name: 'movements', accounting_entry: 'C')
    ...
    expect(transaction_movement.reload.accounting_entry).to eq expected_response[:accounting_entry][:value]
  end

  it 'Show TransactionMovement' do
    transaction_movement = TransactionMovement.create(puc_account_id: PucAccount.last.id , type: 'sale', block_name: 'movements', accounting_entry: 'D')
    visit '/admin/transaction_movements'
    within("#transaction_movement_#{transaction_movement.id}") do
      click_link('View')
    end
